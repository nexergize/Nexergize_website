...
const Header = () => {
  ...
  const handleSmoothScroll = (path: string, e: React.MouseEvent) => {
    // If it's a hash link (section), prevent default and scroll smoothly
    if (path.startsWith('#')) {
      e.preventDefault();
    ...
    <>
      <header
        className={`header top-0 left-0 z-40 flex w-full items-center ${
          sticky
            ? "dark:bg-transparent dark:shadow-sticky-dark shadow-sticky fixed z-9999 bg-white/80 backdrop-blur-xs transition"
                      ...
                      <li key={index} className="group relative">
                        {menuItem.path ? (
                          menuItem.path.startsWith('#') ? (
                            <button
                              onClick={(e) => handleSmoothScroll(menuItem.path, e)}
